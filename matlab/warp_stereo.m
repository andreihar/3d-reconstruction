...
if bb(3)-bb(1)>3000 || bb(4)-bb(2)>3000, 
    error(['XX Error: Your rectification is not correct. ' 'Debug it before going further']) ;
end
...
function A = ivec(v, r); 
%IVEC creates a matrix by grouping the elements of the
%     column vector
%
%A = ivec(v,r) returns a matrix, A, with r rows
%by dividing the vector v into r parts, where each part 
%forms a column of A. It reverses the operation v = A(:),
%if the number of rows of A is known.
%
%Note:
%The number of rows must be a divisor of the size of the vector v.

[n,m]=size(v);
if (m ~= 1 )
    error('The vector v must be a column vector');
end

if (mod(length(v),r)~=0)
    error('The number of rows is not suitable');
end
